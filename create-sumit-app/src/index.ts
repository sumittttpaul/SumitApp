...
async function cleanupGitDirectory(projectPath: string, logger: Logger): Promise<void> {
...
async function selectPackageManager(config: Config, logger: Logger, packageManagerName?: string): Promise<string> {
...
async function selectPresetOrProjects(config: Config, logger: Logger, presetName?: string, projectNames?: string[]): Promise<string[]> {
...
function generatePackageJson(projectName: string, selectedProjects: string[], packageManager: string): object {
...
async function generatePackageManagerConfig(projectPath: string, packageManager: string, logger: Logger): Promise<void> {
...
async function createProject(projectName?: string, options: CreateProjectOptions = {}) {
...
async function addProject(projectName: string, options: { verbose?: boolean }): Promise<void> {
  const logger = new Logger(options.verbose);
...
program
  ...
  .action(async (projectName, options) => {
    ...
    if (options.projects) {
      options.projects = options.projects.split(",").map((p: string) => p.trim());
    }
