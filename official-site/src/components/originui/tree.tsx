...
function TreeItemLabel<T = any>({ item: propItem, children, className, ...props }: TreeItemLabelProps<T>) {
  ...
  return (
    <span
      ...
      className={cn(
        "in-focus-visible:ring-ring/50 bg-code-editor hover:bg-accent in-data-[selected=true]:bg-accent in-data-[selected=true]:text-accent-foreground in-data-[drag-target=true]:bg-accent flex items-center gap-1 rounded-sm px-2 py-1.75 text-sm transition-colors not-in-data-[folder=true]:ps-7 in-focus-visible:ring-[3px] in-data-[search-match=true]:bg-blue-400/20! [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      {...props}
    >
      {item.isFolder() && (
        <ChevronDownIcon
          className={cn("text-muted-foreground size-4 transform transition-all duration-100 ease-in", item.isExpanded() ? "rotate-0" : "-rotate-90")}
        />
      )}
