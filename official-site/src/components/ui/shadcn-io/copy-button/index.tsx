  ...
  "inline-flex items-center justify-center cursor-pointer rounded-md transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
  {
    variants: {
      ...
      size: {
        ...
        text: "h-10 [&_svg]:size-5 w-auto rounded-lg gap-x-2.5 px-3",
      },
    },
    defaultVariants: {
      variant: "default",
...
function CopyButton({ content, className, size, variant, delay = 3000, onClick, onCopy, isCopied, onCopyChange, ...props }: CopyButtonProps) {
  const [localIsCopied, setLocalIsCopied] = useStateX(isCopied ?? false);
  const Icon = localIsCopied.get() ? CheckIcon : CopyIcon;
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        <motion.span
          key={localIsCopied.peek ? "check" : "copy"}
      ...
      {size === "text" && <span className="text-secondary-foreground whitespace-nowrap">Copy code</span>}
    </motion.button>
  );
}
