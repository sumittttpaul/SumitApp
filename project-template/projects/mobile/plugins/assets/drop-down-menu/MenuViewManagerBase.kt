...
abstract class MenuViewManagerBase : ReactClippingViewManager<MenuView>() {
  ...
  fun setHitSlop(view: ReactViewGroup, @Nullable hitSlop: ReadableMap?) {
    if (hitSlop == null) {
      // Use reflection to safely call setHitSlop if it exists
      try {
        val method = view.javaClass.getMethod("setHitSlop", Rect::class.java)
        method.invoke(view, null)
      } catch (e: Exception) {
        // setHitSlop method not available, skip
      }
    } else {
      val rect = Rect(
  ...
  fun setOverflow(view: ReactViewGroup, overflow: String?) {
    // Use reflection to handle different React Native versions
    try {
      val method = view.javaClass.getMethod("setOverflow", String::class.java)
      method.invoke(view, overflow)
    } catch (e: Exception) {
      // setOverflow method not available, try alternative approaches
      try {
        // Some versions might have different method signatures
        val field = view.javaClass.getDeclaredField("overflow")
        field.isAccessible = true
        field.set(view, overflow)
      } catch (e2: Exception) {
        // If neither works, skip this functionality
      }
    }
  }
